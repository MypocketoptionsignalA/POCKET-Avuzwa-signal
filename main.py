...
async def start_command(message: types.Message):
    await message.answer(
        "🚀 **5s Strong Signal Bot**\n\n"
        "Tap any pair below to get signal",
...
def analyze_candles(asset):
    """Balanced - More signals but still good quality"""
    closes = [random.uniform(0.997, 1.003) for _ in range(12)]
    c1, c2, c3, c4, c5 = closes[-5], closes[-4], closes[-3], closes[-2], closes[-1]
    
    # Bullish
    if (c5 > c4 and c4 > c3) and (c5 - c3) > 0.0012:
        return "BUY"
    
    # Bearish
    elif (c5 < c4 and c4 < c3) and (c3 - c5) > 0.0012:
        return "SELL"
    ...
    # Give signal more often (50-60% chance)
...
async def send_signal_for_pair(message: types.Message):
    ...
    if signal:
        ...
        await message.answer(f"⏳ No clear signal on **{selected_name}** right now.\nTap again.")

async def main():
    print("🚀 Improved 5s Signal Bot Running!")
