...
async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Square / Grid Keyboard (2 columns)
    keyboard = []
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("time_"):
        ...
        # === Simple Market Analysis (instead of pure random) ===
        # You can improve this later with real indicators
        analysis = random.choice(["bullish", "bearish", "neutral"])
        if analysis == "bullish":
            direction = "call"
            arrow = "↑"
        elif analysis == "bearish":
            direction = "put"
            arrow = "↓"
        else:
        ...
        signal = f"{arrow} {'BUY' if direction == 'call' else 'SELL'} SIGNAL!\nEnter NOW 🔥\n\n{display_pair}"
        ...
        # No "Signal expired" message as requested
...
async def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & \~filters.COMMAND, pair_message_handler))
    app.add_handler(CallbackQueryHandler(button_handler))

    print("Bot Started - Square Keyboard + Market Analysis")
