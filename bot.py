...
# Updated Pairs (with your new ones)
PAIRS = [
    "🇺🇸🇯🇵 USD/JPY OTC",
    "🇬🇧🇺🇸 GBP/USD OTC",
    "🇬🇧🇯🇵 GBP/JPY OTC",
...
async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Persistent Keyboard at bottom (like your file)
    keyboard = [[KeyboardButton(pair)] for pair in PAIRS]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)

    await update.message.reply_text("📍 Select Pair from keyboard below ⬇️", reply_markup=reply_markup)
...
async def pair_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if selected in PAIRS:
        ...
        await update.message.reply_text("Please select a pair from the keyboard.")
...
async def main():
    ...
    print("🚀 Bot running - Pairs on Persistent Keyboard")
