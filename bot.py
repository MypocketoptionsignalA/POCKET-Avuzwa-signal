...
PAIRS = [
    ...
    "🇪🇺/🇯🇵 EUR/JPY OTC", "🇳🇿/🇺🇸 NZD/USD OTC", "🇪🇺/🇦🇺 EUR/AUD OTC",
    "🇨🇦/🇯🇵 CAD/JPY OTC", "🇬🇧/🇨🇭 GBP/CHF OTC", "🇪🇺/🇨🇭 EUR/CHF OTC",
    "🇳🇿/🇯🇵 NZD/JPY OTC", "🇺🇸/🇨🇦 USD/CAD OTC", "🇨🇭/🇯🇵 CHF/JPY OTC",
    "🇳🇿/🇨🇦 NZD/CAD OTC", "🇨🇦/🇨🇭 CAD/CHF OTC", "🇪🇺/🇳🇿 EUR/NZD OTC"
]
...
async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_USER_ID:
        ...
        return
    keyboard = [[KeyboardButton(PAIRS[i]), KeyboardButton(PAIRS[i+1])] if i+1 < len(PAIRS) else [KeyboardButton(PAIRS[i])] for i in range(0, len(PAIRS), 2)]
...
async def pair_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tf_keyboard = [[InlineKeyboardButton(tf, callback_data=f"time_{tf}")] for tf in TIMEFRAMES]
